+++
title = "A Crash Course on Web Security for The Beginner"
...
- Web application security is a large part of red teaming. With a large percentage of CTFs/Engagements/Bounties being webapp based.
- By being so niche, it can be quicker building up to practical proficiency, this can be fantastic in boosted confidence by getting a quicker return on value.
...
Want to see an internet?! Open up your command prompt/terminal/whatever (google this step if you don't know) and run the following command:``
...
When I make a request to a web server, I will use a method like let's say POST (usually used to edit something). I'll put in the URL I am making the request to. I might also have some HTTP headers which are key value pairs of info like I might have one for authentication like`token: jaslkdfjalkfsjdscd`that proves the request is me. Finally I will have HTTP body information that has what I am submitting.
...
Now most people will start you on the classic OWASP Top 10, you can and should Google that however nay, we are going to go through something which in my opinion is better-er! And this is the [CWE - Software Development List](https://cwe.mitre.org/data/definitions/699.html).
...
- **Broken Access Control:**This occurs when you have a login for a website (Authentication), and those login accounts can do different things (Authorisation) depending on the account that you are logged into. For instance, if there are admin and standard users to an application, you might want to play around with an admin account, and then try to repeat the requests for those actions while logged into the standard user account (remember earlier how we talked about manipulating requests, and not working with only the frontend on the browser). You'd be surprised at how many times an admin feature just checks for a logged in session, and not specifically on if the user making the admin-related request is indeed an admin. This problem only gets worse with the complexity of the application (more roles more problems).
- **Cross-site Scripting:**Cross-site Scripting is where a URL parameter (like we saw before), or information sent in a request is reflected back to the web application and is interpreted as Javascript code. For older web application technology, it is common to see an input consisting of`'/()";`characters and the like being reflected back as code instead of text if formatted properly. This allows hackers to inject code and do malicious things like redirect the user to a phishing page, steal cookies, and make the user do another request.
- **Logical Errors:**This one is hard to categorise, however there is an area of vulnerability where the inherent design of functionality may be not so well thought out. Let's say for a shopping app, if they forget to have a*One coupon per cart*rule, one could submit multiple 5% off coupons and get the item for pennies on the dollar. This example is contrived however for trading applications, gambling sites, market apps, and anything with a complex or contrived flow, doing this sort of hacking where the requests and order can be chopped and changed around could prove to expose very interesting functionality where the useability of the application and what it does is being used to undermine the application.
...
Congratulations if you have read through this far, you're doing awesome and you should feel awesome. Now that you have the basics down and know a few basic hacks, let's tool about getting you some cyber weapons and a range to fire . The first thing you want is a web application proxy attack tool. The defacto standard is Burp Suite by Portswigger which can be found [HERE.](https://portswigger.net/burp/communitydownload) The other contender for the start of your journey is the more open source, less popular, but equally as valid OWASP ZAP, or Zed Attack Proxy, which can be found [HERE.](https://www.zaproxy.org/) There are lots of web application security scanners however these tools are great to start off with instead of using a whole suite of tools due to these proxy attack tools being able to enable you do to multiple things like:
...
The [Portswigger Web Security Academy](https://portswigger.net/web-security) is made by the same people that created the Burp Suite tool. Here you can create a free account. And from here I recommend going through and looking at topics, learning paths, and overall exploring vulnerabilities we discussed previously.
